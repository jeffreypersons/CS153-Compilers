...
if [ $? == 0 ]; then
    ...
    echo "Caused failure in producing object file: $filename.j"
    exit $?
fi
...
if [ $? == 0 ]; then
    ...
    echo "Caused failure in producing class file: $filename.class"
    exit $?
fi
...
if [ $? == 0 ]; then
    ...
    echo "Caused failure in running class file: $filename.class"
    exit $?
fi
