...
void Executor::initialize(istream &data_stream)
{
    if (INITIALIZED) return;

    runtime_stack = MemoryFactory::create_runtime_stack();
    std_in  = new PascalScanner(new Source(data_stream));
    std_out = &cout;

    INITIALIZED = true;
}

Executor::Executor() : debugger(nullptr)
{
}

Executor::Executor(Executor *parent)
    : Backend(), debugger(parent->debugger)
{
}

Executor::Executor(istream &data_stream)
{
    RuntimeError::initialize();
    initialize(data_stream);

    debugger = BackendFactory::create_debugger(COMMAND_LINE, this,
                                               runtime_stack);
}
