...
int main(int argc, char *args[])
{
    try
    {
        ...
        // Source path.
        if (i < argc) source_path = args[i];
        else throw string("Missing source file.");

        // Runtime input data file path.
        if (++i < argc) data_path = args[i];

        Pascal(operation, source_path, data_path, flags);
    }
    catch (string& msg)
    {
        cout << "***** ERROR: " << msg << endl;
...
Pascal::Pascal(string operation, string source_path, string data_path,
               string flags)
    throw (string)
{
    ...
    {
        throw string("Failed to open source file " + source_path);
    }
    ...
    if (data_path != "")
    {
        ...
        if (data_stream.fail())
        {
            throw string("Failed to open data file " + data_path);
        }
    }
    ...
    source = new Source(source_stream);
    ...
    if (parser->get_error_count() == 0)
    {
        ...
        backend = BackendFactory::create_backend(
                    operation, data_path != "" ? data_stream : cin);
