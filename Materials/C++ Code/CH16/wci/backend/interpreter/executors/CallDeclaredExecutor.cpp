...
CellValue *CallDeclaredExecutor::execute(ICodeNode *node)
{
    ...
    if (node->get_children().size() > 0)
    {
        ...
        EntryValue *entry_value =
                    routine_id->get_attribute((SymTabKey) ROUTINE_PARMS);
        vector<SymTabEntry *> formal_ids = entry_value->v;
    ...
    EntryValue *entry_value =
                    routine_id->get_attribute((SymTabKey) ROUTINE_ICODE);
    ICode *icode = entry_value->icode;
    ICodeNode *root_node = icode->get_root();

    // Execute the routine.
    StatementExecutor statement_executor(this);
    CellValue *cell_value = statement_executor.execute(root_node);
    ...
    for (int i = 0; i < formal_ids.size(); ++i)
    {
        ...
        if (formal_defn == (Definition) DF_VALUE_PARM)
        {
            ...
            TypeSpec *value_typespec  =
                                actual_node->get_typespec()->base_type();
